-- Fix stuck broker scans that have been "running" for more than 1 hour
-- These scans have results but were never marked completed
UPDATE broker_scans
...
-- Update the RPC to treat stale running scans as completed
...
BEGIN
  ...
  -- Treat scans stuck in running/pending for >1h as completed
  IF v_broker_status IN ('running', 'pending') AND v_broker_scan_started < now() - interval '1 hour' THEN
    v_broker_status := 'completed';
    v_broker_last_scan := COALESCE(v_broker_last_scan, v_broker_scan_started);
  END IF;
