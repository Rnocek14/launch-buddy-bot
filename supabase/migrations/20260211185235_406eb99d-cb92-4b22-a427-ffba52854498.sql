...
BEGIN
  ...
  IF v_last_email_scan IS NOT NULL THEN
    SELECT count(*)::integer
    INTO v_new_since_last_scan
    FROM user_services
    WHERE user_id = p_user_id
      AND first_discovered_at > v_last_email_scan;
  ELSE
  ...
  -- Count exposed/possible results (matches the frontend filter)
  SELECT count(*)::integer
  INTO v_exposed_result_count
  FROM broker_scan_results
  WHERE user_id = p_user_id
    AND status_v2 IN ('found', 'possible_match');

  -- Top 3 brokers by confidence
  SELECT coalesce(jsonb_agg(row_to_json(t)::jsonb), '[]'::jsonb)
  INTO v_top_brokers
  FROM (
    SELECT
      db.name,
      bsr.status_v2 AS status
    FROM broker_scan_results bsr
    JOIN data_brokers db ON db.id = bsr.broker_id
    WHERE bsr.user_id = p_user_id
      AND bsr.status_v2 IN ('found', 'possible_match')
  ...
  RETURN jsonb_build_object(
    'accounts', jsonb_build_object(
      'count', v_account_count,
      'new_since_last_scan', v_new_since_last_scan,
      'last_scan', v_last_email_scan
    ),
    'brokers', jsonb_build_object(
      'status', v_broker_status,
      'found', v_broker_found,
      'exposed_result_count', v_exposed_result_count,
      'total_checked', v_broker_total,
      'last_scan', v_broker_last_scan,
      'scan_started', v_broker_scan_started,
      'top_brokers', v_top_brokers
    ),
    'breaches', jsonb_build_object(
      'total', v_breach_total,
      'critical', v_breach_critical,
      'high', v_breach_high,
      'last_scan', v_breach_last_scan
    ),
