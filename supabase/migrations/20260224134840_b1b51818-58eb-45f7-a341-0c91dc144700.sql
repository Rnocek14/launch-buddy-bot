-- Clean up duplicate public_results UPDATE policy
DROP POLICY IF EXISTS "Users can update their own results" ON public.public_results;

-- Add auth guard to increment_subscription_count (both overloads)
CREATE OR REPLACE FUNCTION public.increment_subscription_count(p_user_id uuid, p_sender_email text, p_add_count integer, p_last_seen timestamp with time zone, p_subject text DEFAULT NULL::text, p_unsub_url text DEFAULT NULL::text, p_unsub_mailto text DEFAULT NULL::text, p_has_one_click boolean DEFAULT false, p_service_id uuid DEFAULT NULL::uuid, p_connection_id uuid DEFAULT NULL::uuid, p_sender_domain text DEFAULT NULL::text, p_sender_name text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
AS $function$
BEGIN
  -- Auth guard: only the user themselves or service_role can call this
