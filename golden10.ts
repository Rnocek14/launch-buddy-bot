// Run with: deno run --allow-net --allow-read --allow-write golden10.ts
// Or with Node 18+: node golden10.ts
...
const cfg = JSON.parse(await readText('./golden10.config.json')) as {
...
for (const r of results) {
  ...
  console.log(`${status} ${r.domain.padEnd(14)}  ${String(r.time_ms).padStart(5)} ms  ${url}`);
}
console.log('----------------');
console.log(`Pass: ${okCount}/10`);
...
await writeText('./golden10.summary.json', JSON.stringify(summary, null, 2));
